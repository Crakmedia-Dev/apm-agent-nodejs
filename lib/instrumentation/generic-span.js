...
GenericSpan.prototype.duration = function () {
  if (!this.ended) {
    this._agent.logger.debug("tried to call duration() on un-ended transaction/span %o", {
      id: this.id,
...
GenericSpan.prototype.setLabel = function (key, value) {
  ...
  // Until this is fixed: https://github.com/elastic/apm-agent-nodejs/issues/1598
  if (typeof value === "string") {
    this._labels[skey] = truncate(String(value), config.INTAKE_STRING_MAX_SIZE);
  } else {
    this._labels[skey] = value;
  }
  return true;
};
